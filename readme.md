...
- **混合检索**：标题匹配、正文/赏析双路语义检索、作者/朝代硬过滤与标签软打分。
...
1. 精确标题或查询中唯一出现的标题可以直接定位作品。
2. `BAAI/bge-large-zh-v1.5` 分别检索诗文正文与逐段赏析，同一作品取两路最高余弦相似度。
3. 查询中出现的作者、朝代作为硬过滤条件；意象、题材、情感等标签参与软打分。
4. 普通候选按 `0.6 × semantic_score + 0.4 × tag_score` 融合；精确标题、部分标题和语义候选依次排序。
...
| 工具 | 作用 |
| --- | --- |
| `search_poems(query, top_k=5)` | 按标题、语义及标签检索候选作品 |
